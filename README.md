...
- ⚡ **零闪现刷新**：站点设置与产品数据会在浏览器本地缓存，刷新页面时先用缓存即时渲染（不再闪现默认示例内容），再静默从 API 更新——内容变化依然实时生效
...
页脚站点名称与版权年份跟随「品牌」设置自动更新；「产品」列链接跟随后台产品数据自动更新（取前 4 个）；底部提供中英文切换下拉。模板中原本无效的 Careers / Press 等死链已移除。
